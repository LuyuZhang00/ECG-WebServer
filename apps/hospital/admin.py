...
@admin.register(HospitalManage)
class HospitalManage(ImportExportModelAdmin):
    # 设置页面可以展示的字段
    list_display = ('hospital_id', 'hospital_name','hospital_address',
              'hospital_describe','hospital_telephone','create_time','update_time',
              'hospital_SoftLicense','hospital_SoftLicense_used')
    ...
    list_editable = ('hospital_SoftLicense',)
    # 按日期月份筛选 该属性一般不用
    ...
    fieldsets = (
        (None, {
            'fields': ('hospital_id', 'hospital_name','hospital_SoftLicense','hospital_SoftLicense_used')
        }),
        ('选填字段', {
            'classes': ('wide', 'extrapretty',),
            'fields': ('hospital_describe','hospital_telephone','hospital_address',
              ),
        }),
    )
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        return super(HospitalManage, self).change_view(request, object_id,
